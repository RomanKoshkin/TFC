...
class NTXentLoss_poly(torch.nn.Module):
    ...
    def __init__(self, device, batch_size, temperature, use_cosine_similarity, batch_mult=1):
        # pass batch_mult if you rearrange channels into the batch dim
        super(NTXentLoss_poly, self).__init__()
        self.batch_size = batch_size * batch_mult
        self.temperature = temperature
        self.device = device
        self.softmax = torch.nn.Softmax(dim=-1)
        self.mask_samples_from_same_repr = self._get_correlated_mask().type(torch.bool)
    ...
    def forward(self, zis, zjs):
        ...
        onehot_label = torch.cat(
            (torch.ones(2 * self.batch_size, 1), torch.zeros(2 * self.batch_size, negatives.shape[-1])),
