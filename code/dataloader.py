...
class MIdatasetMC(Dataset):

    def __init__(self, dataset, config, transform=None):
        super(MIdatasetMC, self).__init__()
        ...
        cprint(self.x_data.shape, 'green')

        # make FFT
        self.x_data_f = fft.fft(self.x_data).abs()
        assert self.x_data.shape[0] == self.x_data_f.shape[0]

    def __getitem__(self, index):
        if self.training_mode == "pre_train":
            x_t = self.x_data[index]
            x_f = self.x_data_f[index]
            y = self.y_data[index]
            meta = self.meta.iloc[index].to_dict()
            aug_T, aug_1_t, aug_2_t, aug_3_t, msk_t = DataTransform_TD(x_t, self.config)
            aug_F, aug_1_f, aug_2_f, msk_f = DataTransform_FD(x_f, self.config)
            # 'orig_TD', 'label', 'aug_TD', 'orig_freq', 'aug_freq'
            return x_t, y, aug_T, x_f, aug_F, msk_t, msk_f, meta
...
class Load_Dataset(Dataset):
    ...
    def __init__(self, dataset, config, training_mode, target_dataset_size=64, subset=False):
        ...
        cprint('redefined here')  # second dim was :1
        ...
        if training_mode == "pre_train":  # no need to apply Augmentations in other modes
            self.aug1 = DataTransform_TD_old(self.x_data, config)
            self.aug1_f = DataTransform_FD_old(self.x_data_f, config)  # [7360, 1, 90]
...
def data_generator(sourcedata_path, targetdata_path, configs, training_mode, subset=True):
    ...
    cprint('redefined the <data_generator> func.', color='red', attrs=['bold'])
    # train_dataset['samples'] = torch.randn(371055, 2, 178)  # <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
    ...
    if test_dataset['labels'].shape[0] > 10 * configs.target_batch_size:
        test_dataset = Load_Dataset(test_dataset,
                                    configs,
                                    training_mode,
                                    target_dataset_size=configs.target_batch_size * 10,
                                    subset=subset)
    else:
        test_dataset = Load_Dataset(test_dataset,
                                    configs,
    ...
    valid_loader = torch.utils.data.DataLoader(dataset=finetune_dataset,
