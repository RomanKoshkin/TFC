...
def Trainer(model,
    ...
    if training_mode == "pre_train":
        ...
        for epoch in trange(1, config.num_epoch + 1):
            # Train and validate
            """Train. In fine-tuning, this part is also trained???"""
            train_loss, train_acc, train_auc = model_pretrain(
                model,
                model_optimizer,
                train_dl,  # output by <data_generator>
                config,
                device,
                training_mode)
        ...
        os.makedirs(os.path.join(experiment_log_dir, "saved_models"),
                    exist_ok=True)  # only save in self_supervised mode.
    ...
    if (training_mode != "pre_train"):  # no need to run the evaluation for self-supervised mode.
        ...
        for epoch in range(1, config.num_epoch + 1):
            (valid_loss, valid_acc, valid_auc, valid_prc, emb_finetune, label_finetune,
             F1) = model_finetune(model,
                                  valid_dl,
                                  config,
                                  device,
                                  training_mode,
            ...
            (emb_test, label_test, performance) = model_test(model,
                                                             test_dl,
                                                             config,
                                                             device,
                                                             training_mode,
...
def model_pretrain(model, model_optimizer, train_loader, config, device, training_mode):
    total_loss = []
    total_acc = []
    total_auc = []
    model.train()
    ...
    for batch_idx, (data, labels, aug1, data_f, aug1_f) in enumerate(tqdm(train_loader)):
        ...
        # embed the original data in the time and frequency domains.
        # h_t, h_f are time and frequency embeddings (*should* be different for the same orinial sample)
        # z_t, z_f are projections of those embeddings into a joint time-frequency space. They *should* be
        # the same for the same original (or augmented) sample, but different for different samples
        h_t, z_t, h_f, z_f = model(data, data_f)

        # embed the augmented data in the time and frequency domains.
        # same as commented above.
        h_t_aug, z_t_aug, h_f_aug, z_f_aug = model(aug1, aug1_f)

        # Compute Pre-train loss. NTXentLoss: normalized temperature-scaled cross entropy loss. From SimCLR
    ...
    with torch.no_grad():
        ...
        for data, labels, _, data_f, _ in test_dl:
            ...
            if training_mode != "pre_train":
                ...
                auc_bs = roc_auc_score(onehot_label.detach().cpu().numpy(),
                                       pred_numpy,
                                       average="macro",
                                       multi_class="ovr")
