...
def DataTransform_TD(sample, config):
    ...
    aug_1[(1 - li_onehot[:, 0]).astype(bool)] = 0  # the rows are not selected are set as zero.
    aug_2[(1 - li_onehot[:, 1]).astype(bool)] = 0
    aug_3[(1 - li_onehot[:, 2]).astype(bool)] = 0

    # aug_4[1 - li_onehot[:, 3]] = 0 # это было закомментированно в оригинальном коде
